...
namespace hhSalon.Services.Services.Implementations
{
    public class AttendancesService : IAttendancesService
    {
        ...
        public AttendancesService(IUnitOfWork unitOfWork)
        {
        ...
        public async Task AddNewAttendanceAsync(NewAttendanceVM newAttendance)
        {
            ...
            Attendance attendance = new Attendance()
            {
                ...
                Price = _unitOfWork
                        ...
                        .Set
                        .Where(s => s.Id == newAttendance.ServiceId)
                        .Select(s => s.Price)
                        .FirstOrDefault(),
            ...
            await _unitOfWork.GenericRepository<Attendance>().AddAsync(attendance);
        ...
        public async Task<IEnumerable<Attendance>> MyNotRenderedIsPaidAttendances(string userId)
        {
            var attendances = await _unitOfWork
                .GenericRepository<Attendance>()
                .Set
                .Where(a => a.IsRendered == YesNo.No && a.IsPaid == YesNo.Yes && a.ClientId == userId)
                .Include(a => a.Client)
                .Include(a => a.Group)
                .Include(a => a.Service)
                .Include(a => a.Worker)
                .ThenInclude(w => w.User).OrderBy(a => a.Date).ThenBy(a => a.Time)
                .OrderBy(a => a.Date)
                .ToListAsync();
            return attendances;
        }
        ...
        public async Task<IEnumerable<Attendance>> MyNotRenderedNotPaidAttendances(string userId)
        {
            var attendances = await _unitOfWork
                .GenericRepository<Attendance>()
                .Set
                .Where(a => a.IsRendered == YesNo.No && a.IsPaid == YesNo.No && a.ClientId == userId)
                .Include(a => a.Client)
                .Include(a => a.Group)
                .Include(a => a.Service)
                .Include(a => a.Worker)
        ...
        public async Task<IEnumerable<Attendance>> MyIsRenderedAttendances(string userId)
        {
            var attendances = await _unitOfWork
                .GenericRepository<Attendance>()
                .Set
                .Where(a => a.IsRendered == YesNo.Yes && a.ClientId == userId)
                .Include(a => a.Client)
                .Include(a => a.Group)
                .Include(a => a.Service)
                .Include(a => a.Worker)
        ...
        public async Task<IEnumerable<Attendance>> WorkerNotRenderedIsPaidAttendances(string workerId)
        {
            var attendances = await _unitOfWork
                .GenericRepository<Attendance>()
                .Set
                .Where(a => a.WorkerId == workerId && a.IsRendered == YesNo.No && a.IsPaid == YesNo.Yes)
                .Include(a => a.Client)
                .Include(a => a.Group)
                .Include(a => a.Service)
                .Include(a => a.Worker)
        ...
        public async Task<IEnumerable<Attendance>> WorkerNotRenderedNotPaidAttendances(string workerId)
        {
            var attendances = await _unitOfWork
                .GenericRepository<Attendance>()
                .Set
                .Where(a => a.WorkerId == workerId && a.IsRendered == YesNo.No && a.IsPaid == YesNo.No)
                .Include(a => a.Client)
                .Include(a => a.Group)
                .Include(a => a.Service)
                .Include(a => a.Worker)
        ...
        public async Task<IEnumerable<Attendance>> WorkerNotRenderedAttendances(string workerId)
        {
            var attendances = await _unitOfWork
                .GenericRepository<Attendance>()
                .Set
                .Where(a => a.WorkerId == workerId && a.IsRendered == YesNo.No)
                .Include(a => a.Client)
                .Include(a => a.Group)
                .Include(a => a.Service)
                .Include(a => a.Worker)
        ...
        public async Task<IEnumerable<Attendance>> WorkerIsRenderedAttendances(string workerId)
        {
            var attendances = await _unitOfWork
                .GenericRepository<Attendance>()
                .Set
                .Where(a => a.WorkerId == workerId && a.IsRendered == YesNo.Yes)
                .Include(a => a.Client)
                .Include(a => a.Group)
                .Include(a => a.Service)
                .Include(a => a.Worker)
        ...
        public async Task<IEnumerable<TimeSpan>> GetFreeTimeSlots(string workerId, DateTime date)
        {
            ...
            var attendances = _unitOfWork.GenericRepository<Attendance>().Set.Where(a => a.WorkerId == workerId && a.Date == date).ToList();

            TimeSpan start = _unitOfWork
                .GenericRepository<Schedule>()
                .Set
                .Where(s => s.WorkerId == workerId && s.Day == day.ToString())
                .Select(a => a.Start)
                .FirstOrDefault();

            TimeSpan end = _unitOfWork
                .GenericRepository<Schedule>()
                .Set
                .Where(s => s.WorkerId == workerId && s.Day == day.ToString())
                .Select(a => a.End)
        ...
        {
            await _unitOfWork.GenericRepository<Attendance>().DeleteAsync(id);
            await _unitOfWork.SaveChangesAsync();
        }
